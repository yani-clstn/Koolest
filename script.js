  ...
  function initSlideshow() {
    ...
    slideIndex++;
    if (slideIndex > slides.length) {// Hamburger Mobile Nav & Page Interactions
document.addEventListener("DOMContentLoaded", () => {
  const hamburgerBtn = document.getElementById("hamburger-btn") || document.getElementById("mobileMenuBtn");
  const navMenu = document.getElementById("nav-menu") || document.getElementById("mobileMenu");
  const navLinks = navMenu?.querySelectorAll("a");
...
function initChatWidget() {
  ...
  const setChatOpen = (isOpen) => {
    if (!isOpen) {
      if (closeChatBtn) closeChatBtn.blur();
      if (chatInput) chatInput.blur();
    }

    chatPanel.classList.toggle("chat-panel-hidden", !isOpen);
    chatPanel.inert = !isOpen; // Prevents descendant focus when hidden
    openChatBtn.setAttribute("aria-expanded", String(isOpen));
    openChatBtn.classList.toggle("hidden", isOpen);
    if (isOpen) chatInput.focus();
  };
